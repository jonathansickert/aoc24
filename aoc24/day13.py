...
def compute_min_solution_using_cramers_rule(xa: int, ya: int, xb: int, yb: int, xt: int, yt: int) -> tuple[float, float]:
...
def compute_min_cost(xa: int, ya: int, xb: int, yb: int, xt: int, yt: int) -> int:
    ...
    min_solution = compute_min_solution_using_cramers_rule(xa,ya,xb,yb,xt,yt)
    if not min_solution:
        return 0
    na, nb = min_solution
...
def solve_day_1(input: str) -> tuple[int, int]:
    ...
    for machine in input.split("\n\n"):
        ...
        answer2 += compute_min_cost(xa,ya,xb,yb, xt+10000000000000, yt+10000000000000)
